...
func (c *ControllerV1) BatchDelete(ctx context.Context, req *v1.BatchDeleteReq) (res *v1.BatchDeleteRes, err error) {
	if len(req.Ids) == 0 {
		return nil, gerror.New("请选择要删除的角色")
	}

	// 检查角色是否存在
	count, err := dao.AdminRoles.Ctx(ctx).WhereIn(dao.AdminRoles.Columns().Id, req.Ids).Count()
	if err != nil {
		return nil, err
	}
	if count != len(req.Ids) {
		return nil, gerror.New("部分角色不存在")
	}

	// TODO: 检查角色是否被用户使用
	// 这里可以添加检查逻辑，查询是否有用户关联这些角色
	// userCount, err := dao.AdminUsers.Ctx(ctx).WhereIn("role_id", req.Ids).Count()
	// if err != nil {
	//     return nil, err
	// }
	// if userCount > 0 {
	//     return nil, gerror.New("部分角色正在被用户使用，无法删除")
	// }

	// 执行批量删除
	_, err = dao.AdminRoles.Ctx(ctx).WhereIn(dao.AdminRoles.Columns().Id, req.Ids).Delete()
	if err != nil {
		return nil, err
	}
